...
// 渲染图例（简化为"有案件 / 暂无案件"两格 —— 14 省独立配色塞图例里太挤）
function renderLegend() {
  const lang = L();
  const el = document.getElementById('map-legend');
  if (!el) return;
  // 用主色作为"有案件"代表色
  const sampleColor = PROVINCE_COLORS['北京市'] || '#B8956A';
  let html = '';
  html += `<span class="legend-item"><span class="legend-swatch" style="background:${sampleColor};"></span>${I18N[lang].legendActive}</span>`;
