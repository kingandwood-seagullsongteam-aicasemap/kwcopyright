...
function openModal(c) {
  ...
  // 详情弹窗中以原始比例展示完整图片（不裁切）
  const figureHTML = (c.image && c.image.trim())
    ? `<div class="modal-figure"><img src="${c.image}" alt="" loading="lazy" /></div>`
    : '';
...
function buildSuggestionPool() {
  ...
  // 只列有案件的省份（避免大量无关省份噪音）
  const provincesWithCases = [...new Set(CASES.map(c => c.province === '最高人民法院' ? '北京市' : c.province))];
